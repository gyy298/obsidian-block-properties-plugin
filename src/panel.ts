...
export class PropertyPanelView extends ItemView {
	...
	private async renderPanel() {
		...
		const titleRow = header.createEl("div", {
			cls: "block-properties-panel-title-row",
		});
		titleRow.createEl("h4", { text: this.currentFile.basename });

		const refreshBtn = titleRow.createEl("button", {
			text: "↺",
			cls: "block-properties-panel-refresh-btn",
			attr: { "aria-label": "Refresh panel" },
		});
	...
	private renderPropertyValue(
		...
		for (const pos of linkPositions) {
			...
			// Clickable link
			// For block references, initially show ^id, then async resolve to content
			const displayText =
				pos.link.type === "block"
					? `^${pos.link.target}`
					: pos.link.alias || pos.link.target;
			...
			// For block references, async resolve and show actual block content
			if (pos.link.type === "block") {
				linkEl.title = `^${pos.link.target}`;
				getBlockContent(
					this.app,
					pos.link.target,
			...
			linkEl.addEventListener("click", async (e) => {
				...
				if (!success) {
					new Notice(
						`Could not find ${pos.link.type === "note" ? "note" : "block"}: ${pos.link.target}`,
					);
				}
			});
					...
					if (file instanceof TFile) {
						...
						setTimeout(() => {
							...
							if (view) {
								...
								editor.scrollIntoView(
									{
										from: { line: entry.line, ch: 0 },
										to: { line: entry.line, ch: 0 },
									},
									true,
								);
							}
