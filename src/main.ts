...
import { getBlockContent, getAllBlockIds } from "./link-resolver";
...
import { BlockRefSuggest } from "./block-ref-suggest";
...
export default class BlockPropertiesPlugin extends Plugin {
	...
	private blockRefSuggest: BlockRefSuggest | null = null;
	...
	async onload() {
		...
		this.registerView(
			PANEL_VIEW_TYPE,
			(leaf) => new PropertyPanelView(leaf, this),
		);

		// Register the block graph view
		this.registerView(
			GRAPH_VIEW_TYPE,
			(leaf) => new BlockGraphView(leaf, this),
		);

		// Register block-property autocomplete (inside [...])
		this.suggest = new BlockPropertiesSuggest(this);
		this.registerEditorSuggest(this.suggest);

		// Register block-reference autocomplete (triggered by [[^).
		// We prepend to the internal suggests array so it is evaluated
		// BEFORE Obsidian's native [[ link suggest, which would otherwise
		// swallow every [[^ keystroke before our onTrigger can run.
		this.blockRefSuggest = new BlockRefSuggest(this);
		const editorSuggestManager = (this.app.workspace as any).editorSuggest;
		if (editorSuggestManager?.suggests) {
			editorSuggestManager.suggests.unshift(this.blockRefSuggest);
			this.register(() => {
				const i = editorSuggestManager.suggests.indexOf(
					this.blockRefSuggest,
				);
				if (i !== -1) editorSuggestManager.suggests.splice(i, 1);
			});
		} else {
			// Fallback for future Obsidian versions
			this.registerEditorSuggest(this.blockRefSuggest);
		}
		...
		// Pre-warm caches after workspace is ready
		this.app.workspace.onLayoutReady(() => {
			this.backlinkIndex?.buildIndex();
			// Build block-ref cache in background so first [[^ is instant
			void this.blockRefSuggest?.warmCache();
		});
		...
		// Add command to add unique block ID to current/selected lines
		this.addCommand({
			id: "add-unique-block-id",
			name: "Add unique block ID to line(s)",
			editorCallback: async (editor: Editor) => {
				...
				// Get all existing block IDs for uniqueness check
				const existingIds = await this.getExistingBlockIds();
				...
				for (const lineNum of lineNums) {
					...
					if (/\^[\w-]+/.test(lineText)) continue;

					// Generate a unique ID
					const allSoFar = new Set([
						...existingIds,
						...addedThisBatch,
					]);
					const id = this.generateUniqueId(allSoFar);
					addedThisBatch.add(id);

					// Append ^id to end of line
				...
				if (addedCount === 0) {
					new Notice("All selected lines already have a block ID");
				} else {
					// Invalidate both caches so new IDs are picked up
					this.suggest?.invalidateCache();
					this.blockRefSuggest?.invalidateCache();
				}
			},
		});
	...
	private postProcessor(el: HTMLElement, ctx: MarkdownPostProcessorContext) {
		...
		const nodesToProcess: {
			node: Text;
			props: ReturnType<typeof parseBlockProperties>;
		}[] = [];
		...
		for (const { node, props } of nodesToProcess.reverse()) {
			...
			for (const prop of props.reverse()) {
				...
				wrapper.addEventListener("mouseenter", (e) => {
					void this.showReadingTooltip(e.target as HTMLElement, prop);
				});
				...
				if (parent) {
					...
					if (
						this.settings.enableConditionalStyling &&
						this.settings.stylingTarget === "line" &&
						conditionalClasses.length > 0
					) {
						const lineEl = wrapper.closest(
							"p, li, div.markdown-preview-sizer > div",
						);
						if (lineEl) {
	...
	private async showReadingTooltip(
		target: HTMLElement,
		...
		tooltip.className =
			"block-properties-tooltip block-properties-reading-tooltip";
		...
		// Map of value span elements keyed by property index, for async updates
		...
		// Async: resolve block references and update value display
		const activeFile = this.app.workspace.getActiveFile() || undefined;
		for (let i = 0; i < prop.properties.length; i++) {
			const propEntry = prop.properties[i];
			const el = valueEls[i];
			if (!propEntry || !el) continue;
			const linkPos = getLinkPositions(propEntry.value);
			for (const lp of linkPos) {
				if (lp.link.type !== "block") continue;
				try {
					const content = await getBlockContent(
						this.app,
						lp.link.target,
						activeFile,
					);
					...
					if (content) {
						const preview =
							content.length > 40
								? content.slice(0, 40) + "\u2026"
								: content;
						const raw = `^${lp.link.target}`;
						el.textContent = (el.textContent ?? "").replace(
							raw,
							preview,
						);
						el.title = propEntry.value;
					}
				} catch {
	...
	/** Collect all block IDs currently in the vault (cache-first). */
	private async getExistingBlockIds(): Promise<Set<string>> {
		const cached = this.suggest?.getCachedBlockIds();
		if (cached && cached.size > 0) return cached;
		// Cache empty or not yet built – scan vault directly
		const allIds = await getAllBlockIds(this.app);
		return new Set(allIds.keys());
	}

	/**
	 * Generate an 8-char alphanumeric ID that is not in `existingIds`.
	 * Retries on collision (astronomically rare with 8 chars).
	 */
	...
	/** @deprecated Use generateUniqueId() for new code. */
